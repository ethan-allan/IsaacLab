...
class TableTopSceneCfg(InteractiveSceneCfg):
    ...
    kr10 = ArticulationCfg(
        prim_path="{ENV_REGEX_NS}/kr10",
        spawn= sim_utils.UsdFileCfg(
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    # ]
    ee_goals = [[ 0.5, 0.5, 0.5],[0.5,-0.4,0.6],[0.,0,0.5]]
    ...
    robot_entity_cfg = SceneEntityCfg("kr10", joint_names=["joint_.*"], body_names=["link_6"])
    # if args_cli.robot == "franka_panda":
    ...
    while simulation_app.is_running():
        ...
        if count % 1500 == 0:
            ...
            count = 0
            # reset joint state
            ee_pose= robot.data.body_link_state_w[:, robot_entity_cfg.body_ids[0], 0:7]
            ...
            diff_ik_controller.set_command(command=ik_commands, ee_pos=ee_pose[0, 0:3], ee_quat=ee_pose[0, 3:7])
        ...
        goal_marker.visualize(ik_commands[:, 0:3] + scene.env_origins, ee_pose_w[:, 3:7])
